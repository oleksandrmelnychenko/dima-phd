...
\node[dissertation block,text width=3.00cm] (prepare) at (-2.10,0.35)
  {Підготовка об’єкта\\$E_A(\theta_A)$};
\node[dissertation block,text width=2.75cm] (prepared) at (1.20,0.35)
  {Підготовлений об’єкт\\$X_{mm}^{\prime}$};
\node[dissertation block,text width=3.45cm] (conditions) at (4.65,-1.75)
  {Умови прихованої доставки\\$V_f=1$, $D\leq\varepsilon$,\\$S(P_A,G)\geq\tau_A$};
\node[dissertation block,text width=3.30cm] (route) at (4.65,0.35)
  {Послідовність $G$\\$g_j\in R_{web}$};
...
\draw[dissertation arrow] (prepared.east) -- (route.west);
\draw[dissertation arrow] (route.south) -- (conditions.north);
\draw[dissertation arrow] (conditions.south) |- (observations.east);
\end{tikzpicture}
